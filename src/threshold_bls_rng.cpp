...
#error "ThresholdBlsRng requires a real BLS backend. Enable IT_ENABLE_BLST."
#endif
...
#error "IT_ENABLE_RELIC has been deprecated due to incompatible hash_to_curve/DST semantics; use the blst backend instead."
#endif
...
#if defined(IT_ENABLE_RELIC)
// NOTE: The RELIC backend implementation for ThresholdBlsRng has been deprecated and
// intentionally disabled because its hash_to_curve/DST handling was not wire-compatible
// with the IETF BLS12-381 draft and the blst backend. The previous implementation is
// retained in version control history for audit purposes but must not be used.
#endif
