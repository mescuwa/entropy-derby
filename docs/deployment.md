...
4. **Entropy coordinator**
   ...
   - Timelock-encrypted bets: set `BetIntakeConfig::deploymentId` (or export `IT_DEPLOYMENT_ID`/`IT_CHAIN_ID`) per environment so the timelock context becomes `timelockContext:deploymentId[|chainId]`. The engine evaluates the Wesolowski puzzle once per race, keeps the resulting output/proof server-side, hands remote decryptors the proof so they can verify cheaply instead of re-running the puzzle, and still withholds the output from bettor tickets. `TimeLockEncryptor::encrypt` now treats the per-bet context label as a guardrail: non-empty overrides that do not match the race context throw immediately. Tickets from the wrong deployment/chain (or any mismatched label) are rejected before they can be added to the pool, preventing cross-environment bet replay, and bet payloads are authenticated with a server-held MAC key that is independent of the RNG seed.
