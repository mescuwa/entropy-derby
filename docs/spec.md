...
- Ed25519 signing helpers: `publish_log` for transcript roots and liability snapshots (fails closed without a non-empty `IT_DEPLOYMENT_ID` and optionally scopes to `IT_CHAIN_ID`), plus VDF proofs bundled with collaborative entropy, and a timelock encryptor that withholds Wesolowski outputs from ciphertexts so the decryptor must recompute the puzzle, scopes puzzles to `timelockContext:deploymentId[|chainId]:serverSeed`, and records invalid ciphertexts for audit
