...
- Timelock encryption binds each puzzle preimage to `timelockContext:deploymentId[|chainId]:serverSeed`, so tickets scoped to another environment are rejected; encrypted bet intake de-dupes tickets by leaf hash to block replayed ciphertext, skips invalid payloads instead of aborting settlement, and appends every failure to the transcript for forensic review.
